...
vi.mock('../src/services/permissionService.ts', () => ({
  permissionService: {
    setUserActions: vi.fn(),
    getUserActions: vi.fn(() => ({})),
    hasPermission: vi.fn(() => true),
    canViewTeam: vi.fn(() => true),
    canViewTasks: vi.fn(() => true),
    canViewAnnouncements: vi.fn(() => true),
    canViewMembers: vi.fn(() => true),
    canManageTasks: vi.fn(() => true),
    canManageAnnouncements: vi.fn(() => true),
    canAddMembers: vi.fn(() => true),
    fetchUserActions: vi.fn().mockResolvedValue({}),
  },
}))
...
vi.mock('../src/composables/useComponentCache.ts', () => ({
  useComponentCache: vi.fn(() => ({
    ...
    onTeamTabChange: vi.fn(),
  })),
}))

import { permissionService } from '../src/services/permissionService.ts'
import { useComponentCache } from '../src/composables/useComponentCache.ts'
