...
const updateAnnouncement = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:${PORT}/api/teams/${props.teamId}/update/announcements`, {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({
          ...
          createdBy: props.userProps.username,
        }),
      })
      ...
      if (response.ok) {
        ...
        emit('announcement-updated', data.announcement)
      } else {
        error.value = true
        success.value = false
...
<template>
    <v-dialog v-model="props.dialog" max-width="600px" closable>
        <v-card>
            ...
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn @click="emit('update:dialog', false)" 
                    variant="outlined" 
                    :disabled="loading"
                >Cancel</v-btn>
                <v-btn
